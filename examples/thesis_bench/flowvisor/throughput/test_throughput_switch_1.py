...
def config_slice(dp, ctl):
    for n in dp.getNodes().values():
        if n.__type__.name == NodeType.HYPERVISOR.name:
            n.setSlice(name="slice1", ctl_url=ctl)
            time.sleep(2)
            for m in dp.getNodes().values():
                if m.__type__.name == NodeType.SWITCH.name:
                    n.setFlowSpace(name=m.getName(), dpid=m.getDpid(bridge='tswitch0'), prio=10, slice="slice1",
                                   match="dl_vlan=20", slice_perm=7)


if __name__ == '__main__':
    logger = get_logger(__name__)

    output = "/root/results/flowvisor/throughput/switches-1"
    try:
    ...
    hyp = dp.addNode(FlowVisor("hyp"))
    ctl_addr = "tcp:{}:6653".format(ctl.getControlIp())
    hyp_addr = "tcp:{}:6633".format(hyp.getControlIp())
    threads = []
    stats = []
